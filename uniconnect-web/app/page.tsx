...
export default function Home() {
  ...
  const handleSwipe = async (dir: 'left' | 'right') => {
    ...
    if (liked) {
      setLikes(prev => [...prev, targetAlumno.matricula])
    }
    ...
    if (!matchData) {
      setTimeout(() => {
        ...
        if (currentIndex < alumnos.length - 1) {
          ...
          setCurrentPhotoIndex(0) // Reiniciar foto para la sig. persona
        } else {
          fetchAlumnosFromDB()
        }
      }, 300)
      ...
      <div className="fixed inset-0 z-50 bg-gray-900 flex flex-col items-center justify-center font-sans animate-in fade-in duration-500 overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col items-center p-6 text-center">
          ...
          <p className="text-xl md:text-2xl font-bold text-white/90 mb-12 animate-in fade-in slide-in-from-bottom-5 duration-1000 delay-300">
            Tú y {matchData.nombre} se han gustado mutuamente.
          </p>
          ...
          <div className="flex flex-col gap-4 w-full max-w-xs animate-in slide-in-from-bottom-10 fade-in duration-1000 delay-500">
            <button className="w-full py-5 rounded-full font-black text-lg bg-white text-pink-600 hover:bg-gray-100 hover:scale-105 shadow-[0_0_40px_rgba(255,255,255,0.4)] transition-all">
              Enviar Mensaje (Chat)
            </button>
            <button 
              onClick={closeMatchModal}
              className="w-full py-4 rounded-full font-bold text-lg border-2 border-white/30 text-white hover:bg-white/10 transition-colors"
            >
              Seguir Deslizando
            </button>
          </div>
  ...
  if (!alumno) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-gray-50 flex-col font-sans p-6">
        <div className="text-center max-w-sm">
          <div className="w-24 h-24 bg-gray-100 rounded-full flex items-center justify-center mx-auto mb-6">
             <span className="text-4xl">🏜️</span>
          </div>
          <h2 className="text-2xl font-extrabold text-gray-800 mb-2">No hay nadie cerca</h2>
          <p className="text-gray-500 mb-8">Ya viste a todos los estudiantes disponibles por ahora.</p>
          <button onClick={fetchAlumnosFromDB} className="w-full rounded-2xl bg-gradient-to-r from-pink-500 to-violet-500 py-4 text-white font-extrabold text-lg shadow-lg hover:-translate-y-1 transition-all active:scale-95">
            Buscar Nuevos
          </button>
        </div>
      </div>
    ...
    <div className="flex min-h-[100dvh] flex-col items-center bg-gray-50 px-4 pt-4 pb-4 md:pt-8 md:pb-20 font-sans overflow-hidden">
      ...
      <div className="relative w-full max-w-sm flex-1 max-h-[700px] flex items-center justify-center pointer-events-none mb-4">
        <div
          className={`absolute w-full h-full transition-all duration-300 pointer-events-auto ${
            ...
            direction === 'right' ? 'translate-x-[150%] rotate-12 opacity-0' : 'translate-x-0 rotate-0 opacity-100 ease-spring'
          }`}
        >
          <div className="w-full h-full bg-white rounded-[2rem] shadow-[0_20px_50px_rgba(0,0,0,0.08)] border border-gray-100 overflow-hidden flex flex-col relative">
            ...
            <div className="w-full h-[55%] md:h-[60%] bg-gradient-to-br from-pink-400 via-rose-400 to-violet-500 relative flex items-center justify-center shrink-0">
              ...
              {photos.length > 0 ? (
                // eslint-disable-next-line @next/next/no-img-element
                <img src={photos[currentPhotoIndex]} alt={alumno.nombre} className="w-full h-full object-cover" />
              ) : (
                <span className="text-white/40 text-[8rem] font-black tracking-tighter">
              ...
              <div className="absolute bottom-6 left-6 right-6 text-white text-left pointer-events-none">
                ...
                <div className="flex items-center gap-1.5 mt-2 opacity-95 font-semibold text-sm drop-shadow-md">
                   <span>💎</span>
                   <span>{alumno.carrera} ({alumno.semestre}º)</span>
                </div>
              </div>
            </div>
      ...
      <div className="w-full max-w-sm flex justify-center gap-8 mt-auto md:mt-6 shrink-0 z-30">
        <button
          ...
          className="group flex h-20 w-20 items-center justify-center rounded-full bg-white shadow-[0_10px_30px_rgba(0,0,0,0.06)] hover:bg-gray-50 transition-all active:scale-90 border border-gray-100"
        >
        ...
        <button
          ...
          className="group flex h-20 w-20 items-center justify-center rounded-full bg-gradient-to-br from-pink-400 to-rose-500 shadow-[0_10px_30px_rgba(244,63,94,0.3)] hover:shadow-pink-400/50 transition-all active:scale-90"
        >
